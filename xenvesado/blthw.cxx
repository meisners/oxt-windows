...
{
    // nonpaged code called on device interrupt level (DIRQL)
    // to notify VidSch and schedule DPC
...
/*++
  ...
  Routine Description:
    ...
    This routine creates the worker thread to execute present commands
...
--*/
{
    ...
    // Alternate between synch and asynch executon
...
/*++
  ...
  Routine Description:
    ...
    The routine call back OS to report progress of present execution
    ...
    HANDLE Context)
{
    ...
    KeAcquireGuardedMutex(displaySource->m_DevExt->FrameBufferGuard()); 
    if (displaySource->m_DevExt->IsDriverActive())
        HwExecutePresentDisplayOnly(Context);
    KeReleaseGuardedMutex(displaySource->m_DevExt->FrameBufferGuard());
}
