                                           ...
                                           _Out_ ULONG*             pNumberOfChildren)
{
    ...
    m_VBEModeInfo = NULL;
    m_CurrentVBEMode = 0;
    KeInitializeGuardedMutex(&m_FBGuard);
...
//    Status = CheckHardware();
    ...
    if (!NT_SUCCESS(Status) || m_CurrentModes[0].DispInfo.Width == 0)
    {
        ...
        //Set the VBE Mode to 32 bpp.  Improves performance.
        if (m_CurrentModes[0].DispInfo.ColorFormat !=  D3DDDIFMT_A8R8G8B8) {
            if (!m_VBEModeInfo)
            {
                XenGfxVbeInitialize(NULL);
                InitializeVBEModes(); 
            }
            m_CurrentModes[0].SrcModeWidth = m_CurrentModes[0].DispInfo.Width;
            m_CurrentModes[0].SrcModeHeight = m_CurrentModes[0].DispInfo.Height;
            MapVBEFrameBuffer(&m_CurrentModes[0], 0);
        }
    }
    m_Flags.DriverStarted = TRUE;
   *pNumberOfViews = MAX_VIEWS;
   *pNumberOfChildren = MAX_CHILDREN;
   LeaveFunction(__FUNCTION__, 1);
...
NTSTATUS BASIC_DISPLAY_DRIVER::GetEdid(D3DDDI_VIDEO_PRESENT_TARGET_ID TargetId)
{
    ...
    if (!XenGfxVbeGetEdid(m_EDIDs[TargetId], EDID_V1_BLOCK_SIZE)) 
        return STATUS_UNSUCCESSFUL;
...
NTSTATUS BASIC_DISPLAY_DRIVER::FindMode( D3DDDI_VIDEO_PRESENT_SOURCE_ID SourceId, USHORT * ModeNumber )
{
    PAGED_CODE();
    USHORT		Index = 0;

    //Search for best match
    for (Index = 0 ; Index < (USHORT) m_VBEModeCount; ++Index)
    {
        PVBE_MODE_INFO VbeModeInfo = &m_VBEModeInfo[Index].VBEModeInfo;
        if ((VbeModeInfo->XResolution == m_CurrentModes[SourceId].SrcModeWidth) 
            &&(VbeModeInfo->YResolution == m_CurrentModes[SourceId].SrcModeHeight))
        {
            break;
        }
    }

    if (Index == m_VBEModeCount) {
        BDD_LOG_ERROR0("Failed to find valid mode");
        return STATUS_UNSUCCESSFUL;
    }

    *ModeNumber = m_VBEModeInfo[Index].ModeNumber;
    return STATUS_SUCCESS;
}

NTSTATUS BASIC_DISPLAY_DRIVER::InitializeVBEModes()
{
    PAGED_CODE();
    ...
    XenGfxVbeGetExtInfo(&VbeExtInfo, &m_VBEModeCount);

    if (m_VBEModeInfo) {
        ExFreePool(m_VBEModeInfo);
    }

    m_VBEModeInfo = (PVBEModeInfo) operator new (sizeof(VBEModeInfo)*m_VBEModeCount, NonPagedPoolNx);
    if (!m_VBEModeInfo) {
        BDD_LOG_ERROR1("ExallocatePool failed in %s", __FUNCTION__);
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    for (SHORT ModeIndex = 0; ModeIndex < (SHORT) m_VBEModeCount; ++ModeIndex)
    {
        USHORT *		pModeNumber = (USHORT*)(((UCHAR*)&VbeExtInfo) + VBE_VESA_MODE_POINTER_OFFSET);
        if (!XenGfxVbeGetModeInfo (*(pModeNumber + ModeIndex), &m_VBEModeInfo[ModeIndex].VBEModeInfo))
            return STATUS_UNSUCCESSFUL;
        m_VBEModeInfo[ModeIndex].ModeNumber = *(pModeNumber + ModeIndex);
    }

    return STATUS_SUCCESS;
}

void BASIC_DISPLAY_DRIVER::GetPreferredResolution( ULONG ChildUid, DWORD * XRes, DWORD * YRes )
{
    PAGED_CODE();
    if (!m_VBEModeInfo) {
        XenGfxVbeInitialize(NULL);
    ...
    *XRes = (DWORD) (m_EDIDs[ChildUid][58]>>4)<<8 | m_EDIDs[ChildUid][56];
    *YRes = (DWORD) (m_EDIDs[ChildUid][61]>>4)<<8 | m_EDIDs[ChildUid][59];
}

NTSTATUS BASIC_DISPLAY_DRIVER::MapVBEFrameBuffer( CURRENT_BDD_MODE* pCurrentMode, const D3DDDI_VIDEO_PRESENT_SOURCE_ID sourceId )
{
    PAGED_CODE();
    ...
    BDD_ASSERT_CHK(pCurrentMode->FrameBuffer.Ptr == NULL);
    Status = FindMode(sourceId, &ModeNumber);

    if (!NT_SUCCESS(Status))
        return Status;

    if (ModeNumber != m_CurrentVBEMode || !pCurrentMode->FrameBuffer.Ptr)
        XenGfxVbeSetMode(ModeNumber);

    XenGfxVbeGetCurrentMode(&XRes, &YRes, &BPP);
    YRes++;
    pCurrentMode->DispInfo.Pitch =  XenGfxVbeGetAlignedStride(XRes * BPP/BITS_PER_BYTE); 
    pCurrentMode->DispInfo.Width = XRes;
    pCurrentMode->DispInfo.Height = YRes;
    pCurrentMode->DispInfo.ColorFormat = (BPP==32)? D3DDDIFMT_A8R8G8B8 : D3DDDIFMT_R8G8B8;
    KeAcquireGuardedMutex(&m_FBGuard); 
    Status = MapFrameBuffer(pCurrentMode->DispInfo.PhysicAddress,
        pCurrentMode->DispInfo.Pitch * YRes,
        &(pCurrentMode->FrameBuffer.Ptr));
    KeReleaseGuardedMutex(&m_FBGuard);
    return Status;
}

void BASIC_DISPLAY_DRIVER::EnterFunction( const char * Function, int Level )
{
